...
export const KeyboardShortcutsHelp: React.FC = () => {
  const { shortcuts, showHelp, setShowHelp, formatShortcut } =
    useKeyboardShortcuts();

  const groupedShortcuts = {
    Navigation: shortcuts.filter(
      (s) => s.description.startsWith("Go to") && !s.shiftKey
    ),
    Actions: shortcuts.filter(
      (s) => s.description.includes("Add") || s.description.includes("Refresh")
    ),
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="pt-4 border-t border-border">
            <p className="text-sm text-muted-foreground">
              💡 Tip: These shortcuts work from anywhere in the app, except when
